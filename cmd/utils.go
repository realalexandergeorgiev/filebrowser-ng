...
func withViperAndStore(fn func(cmd *cobra.Command, args []string, v *viper.Viper, store *store) error, options storeOptions) cobraFunc {
	return func(cmd *cobra.Command, args []string) error {
		...
		db, err := boltapi.Open(path, databasePermissions, nil)
		if err != nil {
			return err
		}
		defer db.Close()
...
func marshal(filename string, data interface{}) error {
	// Exports carry secrets (signing keys, password hashes, auther
	// credentials): never leave them world-readable, including when
	// overwriting a file that already exists with wider permissions.
