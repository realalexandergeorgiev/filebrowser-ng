...
- **Server-side sessions:** short access JWT (`alg` pinned to HS256, `exp` required, `iss` verified, `jti` checked against the server-side session store, sliding with a max lifetime). Revocation on logout, password change, permission/scope change, admin revoke, user delete.
...
9. `v0.6.0-ng`: backlog batch — Go fuzz targets (+ regex-rule validation), JWT `sub`/`nbf` binding, metadata-op guard adjacency, rebrand leftovers, fork CI, i18n dead-key cleanup.
