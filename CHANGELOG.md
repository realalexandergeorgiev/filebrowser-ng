...
* Settings and auth without storm (`storage/bolt/config.go`, `auth.go`, `kv.go`): same pattern for the shared `config` bucket; compat tests prove both directions. The storm key/value helpers are gone.
