...
* App shell CSP (`http/static.go`, `frontend/`): the strict `default-src 'self'` policy blocked the inline `window.FileBrowser` bootstrap script and `style-src 'unsafe-inline'` (without `'self'`) blocked the external stylesheet, so browsers hung on the loading screen. The index now sends a nonce-based policy (`script-src 'self' 'nonce-…'`, `style-src 'self' 'unsafe-inline'`, `manifest-src 'self' blob:`) and the bootstrap script carries the matching nonce; the global default CSP also gained `script-src 'self'`. Removed `@vitejs/plugin-legacy` (its inline detection scripts cannot be nonced and are unnecessary for modern browsers). Regression test asserts the nonce matches between header and script.
* Sidebar credits link pointed at `github.com/filebrowser-ng/filebrowser-ng`; fixed to `github.com/realalexandergeorgiev/filebrowser-ng`.
...
* TOCTOU on file opens (`files/scoped.go`, `verify_linux.go`): the guard resolved symlinks and the open re-resolved them, so a swap in between could serve outside content. Opened descriptors are now re-validated via `/proc/self/fd` (Linux, OsFs-backed scopes incl. rebased shares): positive proof of escape closes and rejects, everything else defers to the guard, so absolute internal links and exotic environments keep working. Swap-race and descriptor tests prove the invariant.
* HttpOnly session cookies (`http/auth.go`, `frontend/`): the access token no longer lives in `localStorage` or readable cookies, so XSS cannot steal it. Login/renew set `HttpOnly; SameSite=Strict` cookies (`Secure` on TLS); the cookie authenticates all methods (header auth kept for API clients); new `GET /api/auth/me` serves the user plus session expiry for the idle timer; logout revokes and clears. Frontend sends `credentials: same-origin` everywhere (tus/XHR included, same-origin needs no extra flag) and holds no token. CSRF stays out via Strict + no CORS. Tests on both sides; `jwt-decode` dependency dropped.
...
* Share token lifetime (`http/public.go`, `http/share.go`): URL tokens worked forever once leaked. Tokens now carry a creation timestamp, die after 24h without use, and a correct password slides them again; empty tokens no longer bypass password links. Pre-upgrade links need one password entry. Tests cover fresh/stale/legacy/empty tokens.
* Archives without mholt (`http/raw.go`, frontend download UI): the deprecated packing lib is replaced by stdlib zip/tar/gzip writers; exotic codecs (bz2, xz, lz4, sz, br, zst) are rejected with 400 and removed from the UI and types. Round-trip tests read back every format; ~70 dependency lines shed.
...
* Storm dropped (`go.mod`, `storage/bolt/`, tests): with all backends on raw bbolt, the archived ORM, its opener in `cmd` and all test fixtures are gone. A golden `testdata/legacy-v2.db` (written by storm as v2.63 wrote it) pins legacy readability for every backend; `govulncheck` still clean.
...
* Server-side sessions (`sessions/`, `http/auth.go`, `http/users.go`): every access token carries a session ID (`jti`) that must exist server-side, belong to the token user and still be live — unknown, foreign or expired sessions are `401`. New `DELETE /api/logout` revokes the session (frontend calls it best-effort on logout); renew slides the session up to a 30-day max lifetime, past which login is required; password/security updates and user deletion revoke all of the target's sessions; tokens for deleted users are `401` (was `500`). Pre-session tokens without `jti` are rejected (upgrade logs everyone out once). Tests: `sessions/` unit tests, `http/sessions_test.go` end-to-end (login/logout/renew/forgery/delete/password/cosmetic), PoC flipped to assert revocation, proxy-expiry test now covers session-backed waiver.
...
* TUS delete parity (`http/tus_handlers.go`): deletes skipped the descendants rules check plus share/thumbnail cleanup that resource deletes perform, so a file-turned-directory could take denied children with it. Same hardening as resource deletes; regression test deletes through a swapped-in denied tree.
...
* ReCaptcha verification (`auth/json.go`): the HTTP client had no timeout, so a hanging host blocked logins. Bounded at 10s (overridable in tests); test hangs the server and asserts fast failure.
* Router without gorilla (`http/http.go`, `http/users.go`, `http/preview.go`): archived `gorilla/mux` replaced by the stdlib `ServeMux` (method patterns, `{id}`/`{path...}` wildcards, `PathValue`); per-verb dispatch with 405+Allow preserves the old method semantics that a catch-all would otherwise swallow. `gorilla/websocket` and `go-shlex` were already unreferenced and left with the deletions. New `http/routing_test.go` pins the table (fallback, auth, 401s, 301 parity, 405s); `go.mod`/`go.sum` tidied.
* Unmaintained `go-homedir` replaced by stdlib `os.UserHomeDir` (single use).
* Frontend i18n (`frontend/src/i18n/en.json`): removed 11 dead keys of the deleted command execution UI (source only; other locales follow via Transifex).
...
* Command execution UI removed (`frontend/`): deleted the shell terminal, WS command client, hook-runner settings forms, per-user commands field, execute permission checkbox and shell button; dropped `enableExec` plumbing. Verified with `vue-tsc`, `eslint` and `vitest` (29 tests). Unused i18n keys stay for now.
